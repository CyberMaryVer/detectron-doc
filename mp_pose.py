...
from mediapipe_tools import get_updated_keypoint_dict, draw_joints, draw_skeleton
from mediapipe_tools import MpipePredictor, visualize_keypoints_mp
from custom_tools import save_results_to_csv, draw_box_with_text
...
def get_parser():
    ...
    parser.add_argument("--debug_video", default=(5, 100), nargs="+",
                        help="A list of space separated parameters - step and number of frames to process.")

    # OTHER
    parser.add_argument('--exercise', default=None, type=str, help='Exercise to track in real time mode.')
...
if __name__ == "__main__":
    ...
    if _WEBCAM and _EXERCISE is None:
        _EXERCISE = "squats"
    ...
    if _IMAGE is not None:
        ...
        if _TEST:
            ...
            im = draw_skeleton(im, keypoints=kps, threshold=.5, headless=True)
            im, _ = draw_joints(im, keypoints=kps, threshold=.5, headless=True)
            end_time = time()
            ...
            kps = get_updated_keypoint_dict(outputs)
            output_image = visualize_keypoints_mp(kps, im, skeleton=_CONNECTIONS, dict_is_updated=True,
        ...
        if _WEBCAM:
            ...
            while cv2.waitKey(1) != 27:
                ...
                try:
                    ...
                    print(len(outputs))
                    kps = get_updated_keypoint_dict(outputs)
                    frame = visualize_keypoints_mp(kps, frame, skeleton=_CONNECTIONS, dict_is_updated=True,
                                                   threshold=.8, side=_SIDE, mode=_MODE, scale=_SCALE, joints=_JOINTS)
                    print("Success")
                except:
                    pass
            ...
            video = MpipePredictor(path_to_video=_DEBUG_PATH_TO_VIDEO, detection_thr=THRESHOLD)
            total = video.num_frames if debug_params is None else debug_params[1]
