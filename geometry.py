...
def convert_dict(tensor_dict):
    ...
    """
    converted_dict = {}
    for keyp_name, keyp_coords in tensor_dict.items():
        converted = {keyp_name: [coord.flatten().tolist()[0] for coord in keyp_coords]}
...
def draw_angles(keypoint_dict, side=None):
    ...
    visible_right = ['right_elbow', 'right_knee', 'right_shoulder', 'right_hip_center', 'right_neck_center', 'right_hip']
...
if __name__ == "__main__":
    ...
    converted_dict = convert_dict(test_dict)
    # print(draw_angles(converted_dict))
    new_dict = get_updated_keypoint_dict(converted_dict)
